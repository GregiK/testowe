...
- Własny `server.js` (Passenger-compatible entrypoint). Brak `postinstall` - `prisma
  generate` uruchamiany wyłącznie ręcznie (przycisk "Uruchom skrypt JS"), `.next` budowany
  w GitHub Actions (patrz sekcja Deployment, wnioski z rozwiązywania problemów wdrożenia).
...
## Deployment (Aderlo Cloud - pełna automatyzacja przez Git)
...
przez `.htaccess`: `PassengerStartupFile server.js`).

**Historia problemu (dla przyszłych etapów):** pierwsze podejście zakładało budowanie
(`npm install`, `prisma generate`, `next build`) bezpośrednio na serwerze przez przyciski w
panelu ("Run NPM Install", "Uruchom skrypt JS"). Po kolei napotkane problemy: (a) przycisk
"Run NPM Install" okazał się wadliwie działać w panelu Aderlo - tworzył plik `.lock`, ale
nigdy realnie nie wywoływał `npm` (potwierdzone nawet na świeżo odtworzonej instancji
aplikacji); (b) `postinstall` łączący `prisma generate && next build` zawiesił proces na
serwerze na ok. 21 godzin; (c) nawet ręczne `next build` przez Terminal kończyło się błędem
`EAGAIN`/`ERR_WORKER_INIT_FAILED` (Turbopack/webpack próbują odpalać dodatkowe procesy
systemowe, a CloudLinux/LVE tego nie pozwala). Stąd finalna, w pełni zautomatyzowana
procedura poniżej - build dzieje się w GitHub Actions, serwer tylko odbiera gotowe pliki.

**Aktualna, docelowa procedura (pełna automatyzacja - panel Aderlo, zakładka "Wdrożenia"):**

1. **Jednorazowa rejestracja aplikacji w "Node.js Selector"** - folder `public_html`, plik
   startowy `server.js`. Już zrobione (istniejąca aplikacja na test.gpczarnecki.pl).
2. **Zmienne środowiskowe w Node.js Selector** (`DATABASE_URL`, `SESSION_SECRET`) - nigdy w
   repo, przetrwają każde wdrożenie. Jednorazowa konfiguracja przez użytkownika.
3. **Build dzieje się wyłącznie w GitHub Actions** (`.github/workflows/deploy.yml`):
   `npm ci` -> `prisma generate` (z jawnymi `binaryTargets` pod CloudLinux/RHEL, patrz
   `docs/assumptions.md`) -> `next build --webpack` -> `npm prune --omit=dev` -> spakowanie
   kompletnego, gotowego do uruchomienia katalogu (kod + `node_modules` produkcyjne + `.next`
   + świeży `tmp/restart.txt`) i publikacja na osobną gałąź `deploy`.
4. **Brak `postinstall` w `package.json`** - żadnego hooka mogącego zawiesić proces na
   serwerze.
5. **"Automatyczne wdrożenie z Gita" w panelu Aderlo podpięte na gałąź `deploy`** (nie
   `main`!), katalog docelowy `domains/test.gpczarnecki.pl/public_html`. Panel po każdym
   pushu na tę gałąź klonuje ją 1:1 na serwer - nic nie buduje, tylko kopiuje gotowe pliki.
6. **Restart aplikacji następuje automatycznie** - świeża data w `tmp/restart.txt` (część
   pakietu z kroku 3) sygnalizuje Passengerowi zmianę i wymusza restart przy kolejnym żądaniu.
7. Migracje bazy (`npx prisma migrate deploy`) - jedyny krok bez pełnej automatyzacji na razie,
   wymaga jednorazowego/okazjonalnego uruchomienia przez Terminal w panelu przy zmianach
   schematu bazy.

**Efekt końcowy:** zwykły `git push origin main` na komputerze użytkownika uruchamia cały
łańcuch (GitHub Actions buduje -> publikuje na `deploy` -> panel Aderlo wdraża -> restart) bez
żadnego ręcznego klikania przy kolejnych zmianach. Jedyne jednorazowe kroki to: podpięcie
gałęzi `deploy` w panelu (krok 5) i ustawienie zmiennych środowiskowych (krok 2).
